...
export default async function handler(req, res) {
  ...
  const { teamId, index, challenge, points } = await parseBody(req);
  if (!teamId || typeof index !== 'number' || typeof points !== 'number') {
  ...
  try {
    ...
    const oldPts = Number(history[index].pts || 0);
    history[index] = {
      ...history[index],
      challenge,
      pts: points
    ...
    await supa(`teams?id=eq.${teamId}`, {
      method: 'PATCH',
      body: JSON.stringify({
        score: (team.score || 0) - oldPts + points,
        history
      })
    });
